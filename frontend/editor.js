...
function buildCameraMeshes(d) {
  ...
  pivot.rotation.y=THREE.MathUtils.degToRad(-(d.yaw||0));
...
function buildFrustum(d) {
  ...
  const pitchBackend = -(90 + pitch);
  const fx=(iw/2)/Math.tan(THREE.MathUtils.degToRad(fov/2));
  const cx=iw/2,cy=ih/2;
  const yR=THREE.MathUtils.degToRad(yaw||0);
  const pR=THREE.MathUtils.degToRad(pitchBackend);
  function pixelRay(u, v) {
    const ndx = (u - cx) / fx;
    const ndy = (v - cy) / fx;  // убираем минус — в mapper.py нет инверсии
    const ray_cam = [ndx, ndy, 1.0];

    // Rx (pitch вокруг X)
    const cosP = Math.cos(pR), sinP = Math.sin(pR);
    const rx1 = ray_cam[0];
    const ry1 = cosP * ray_cam[1] - sinP * ray_cam[2];
    const rz1 = sinP * ray_cam[1] + cosP * ray_cam[2];

    // Rz (yaw вокруг Z)
    const cosY = Math.cos(yR), sinY = Math.sin(yR);
    const rx_w = cosY * rx1 - sinY * ry1;
    const ry_w = sinY * rx1 + cosY * ry1;
    const rz_w = rz1;

    return new THREE.Vector3(rx_w, rz_w, ry_w).normalize();
  }
...
function snapObjToObj(px, pz, movingEntry) {
  const OBJ_SNAP_DIST = 0.6;
  const d = movingEntry.data;
  const halfWm = (d.w || 1) / 2;
  const halfDm = (d.d || 0.5) / 2;
  const radM = THREE.MathUtils.degToRad(d.rot || 0);
  const cosM = Math.cos(radM), sinM = Math.sin(radM);

  const movingFaces = [
    { nx:  cosM, nz: -sinM, off: halfWm },  // правая
    { nx: -cosM, nz:  sinM, off: halfWm },  // левая
    { nx: -sinM, nz: -cosM, off: halfDm },  // передняя
    { nx:  sinM, nz:  cosM, off: halfDm },  // задняя
  ];

  let best = null, bd = OBJ_SNAP_DIST;

  for (const en of objects) {
    if (en === movingEntry) continue;
    if (en.type !== "shelf" && en.type !== "counter") continue;

    const od = en.data;
    const halfWo = (od.w || 1) / 2;
    const halfDo = (od.d || 0.5) / 2;
    const radO = THREE.MathUtils.degToRad(od.rot || 0);
    const cosO = Math.cos(radO), sinO = Math.sin(radO);

    const staticFaces = [
      { nx:  cosO, nz: -sinO, off: halfWo },
      { nx: -cosO, nz:  sinO, off: halfWo },
      { nx: -sinO, nz: -cosO, off: halfDo },
      { nx:  sinO, nz:  cosO, off: halfDo },
    ];

    for (const mf of movingFaces) {
      const mfx = px + mf.nx * mf.off;
      const mfz = pz + mf.nz * mf.off;

      for (const sf of staticFaces) {
        const dot = mf.nx * sf.nx + mf.nz * sf.nz;
        if (dot > -0.7) continue;

        const sfx = od.x + sf.nx * sf.off;
        const sfz = od.z + sf.nz * sf.off;

        const dist = Math.hypot(mfx - sfx, mfz - sfz);
        if (dist < bd) {
          bd = dist;
          best = {
            x: parseFloat((od.x + sf.nx * sf.off - mf.nx * mf.off).toFixed(2)),
            z: parseFloat((od.z + sf.nz * sf.off - mf.nz * mf.off).toFixed(2)),
          };
        }
      }
    }
...
function computeBlindSpots(camEntry) {
  ...
  const pitchB = -(90 + (d.pitch || 0));
  const pR = pitchB * Math.PI / 180;
  const yR = (d.yaw || 0) * Math.PI / 180;
  const cosP = Math.cos(pR), sinP = Math.sin(pR);
  const cosY = Math.cos(yR), sinY = Math.sin(yR);
  const ry1 = -sinP, rz1 = cosP;
  const dirX = cosY * 0 - sinY * ry1;
  const dirZ = sinY * 0 + cosY * ry1;

  const cam = {
    x: d.x, z: d.z, height: d.height || 3.0,
    yaw: d.yaw || 0, pitch: pitchB,
    fov: d.fov || 90,
  ...
  console.log("cam dir:", cam.dirX, cam.dirZ, "fov:", cam.fov);
  console.log("shadowBoxes:", shadowBoxes.length, "wallBoxes:", wallBoxes.length);
...
function showProps(en) {
  ...
  if (en.type==="camera") {
    ...
    document.getElementById("p-cam-x").value = d.x ?? 0;
    document.getElementById("p-cam-z").value = d.z ?? 0;
    ...
    document.getElementById("p-shelf-x").value = d.x ?? 0;
    document.getElementById("p-shelf-z").value = d.z ?? 0;
    ...
    document.getElementById("p-counter-x").value = d.x ?? 0;
    document.getElementById("p-counter-z").value = d.z ?? 0;
    ...
    document.getElementById("p-wobj-x").value = d.x ?? 0;
    document.getElementById("p-wobj-z").value = d.z ?? 0;
...
bindLive("p-cam-x", "x", parseFloat);
bindLive("p-cam-z", "z", parseFloat);
bindLive("p-shelf-x", "x", parseFloat);
bindLive("p-shelf-z", "z", parseFloat);
bindLive("p-counter-x", "x", parseFloat);
bindLive("p-counter-z", "z", parseFloat);
bindLive("p-wobj-x", "x", parseFloat);
bindLive("p-wobj-z", "z", parseFloat);
...
let _snapLockedPt = null;
const SNAP_HYSTERESIS = SNAP_R * 1.8;
...
window.addEventListener("mousemove",e=>{
  ...
  if (dragGizmo&&gizmoTarget&&hasDragged) {
    ...
    if (dragGizmo==="x") {
      ...
      if (gizmoTarget.type==="wall") {
        snapPts.length = 0;
        for (const o of objects) {
          if (o.type === "wall") { const ep=wallEndpoints(o.data); addSnapPt(ep.ax,ep.az); addSnapPt(ep.bx,ep.bz); }
        }
      }
    } else if (dragGizmo==="z") {
    ...
    const _pfx = gizmoTarget.type === "wall" ? "wobj" : gizmoTarget.type === "camera" ? "cam" : gizmoTarget.type;
    const _xEl = document.getElementById(`p-${_pfx}-x`);
    const _zEl = document.getElementById(`p-${_pfx}-z`);
    if (_xEl) _xEl.value = gizmoTarget.data.x;
    if (_zEl) _zEl.value = gizmoTarget.data.z;
  ...
  if (dragEntry && hasDragged) {
    ...
    if (dragEntry.type === "door") {
      ...
      const sw = snapObjToWall(pt.x, pt.z, dragEntry.data.d);
      if (sw) { nx = sw.x; nz = sw.z; nrot = sw.rot; }

      const so = snapObjToObj(nx, nz, dragEntry);
      if (so) {
        if (sw) {
          const wallRad = THREE.MathUtils.degToRad(sw.rot || 0);
          const wnx = -Math.sin(wallRad), wnz = -Math.cos(wallRad);
          const dx = so.x - nx, dz = so.z - nz;
          const dot = dx * wnx + dz * wnz;
          nx = parseFloat((nx + dx - dot * wnx).toFixed(2));
          nz = parseFloat((nz + dz - dot * wnz).toFixed(2));
        } else {
          nx = so.x; nz = so.z;
        }
      }
      ...
      for (const sign of [1, -1]) {
        ...
        if (_snapLocked && _snapLockedPt) {
          const distFromLock = Math.hypot(testX - _snapLockedPt.x, testZ - _snapLockedPt.z);
          if (distFromLock < SNAP_HYSTERESIS) {
            nx = _snapLockedPt.x - sign * edx;
            nz = _snapLockedPt.z - sign * edz;
            snapped = true;
            break;
          }
        }
    ...
    const _pfx2 = dragEntry.type === "wall" ? "wobj" : dragEntry.type === "camera" ? "cam" : dragEntry.type;
    const _xEl2 = document.getElementById(`p-${_pfx2}-x`);
    const _zEl2 = document.getElementById(`p-${_pfx2}-z`);
    if (_xEl2) _xEl2.value = dragEntry.data.x;
    if (_zEl2) _zEl2.value = dragEntry.data.z;
    updateGizmoPos();
  }
});
